...
-   **Newsletter Generation**: Creates a Markdown summary digest of selected bookmarks ("The Librarian").
...
6.  **Generate Newsletter / Summary**: Summarizes bookmarks into a digest.
7.  **Cleanup Tags**: Deduplicates tags (e.g., merges singular/plural).
8.  **Prune Infrequent Tags**: Deletes tags used less than X times.
9.  **Flatten Library**: Moves all bookmarks to "Unsorted" and optionally deletes empty folders.
10. **Delete ALL Tags**: Nuclear option to remove every tag.

### Safety & Control
-   **The Archivist**: Automatically checks Wayback Machine for broken links and saves the snapshot URL.
...
-   **Cost Tracking**: Real-time display of token usage and estimated API cost (Model-aware).
...
-   **Tag Broken Links**: Checks if the URL is accessible. If broken, checks Wayback Machine for a snapshot.
...
1.  Select **Collection**: "Unsorted" or filter by `#newsletter`.
2.  Select **Mode**: "Generate Newsletter / Summary".
3.  Click **Start**.
4.  Copy the generated Markdown report.
